...
export function OnboardingForm() {
  ...
  const next = () => { const result = validateOnboardingDraft(draft); setDraft(result.draft); setErrors(result.errors); if (!Object.keys(result.errors).length) setStage((value) => Math.min(3, value + 1)) }
  const submit = async () => {
...
function Field({ label, error, children }: { label: string; error?: string; children: React.ReactNode }) { return <div><label className="text-sm font-medium text-admin-ink">{label}</label><div className="mt-2 [&_input]:w-full [&_input]:rounded [&_input]:border [&_input]:border-admin-border [&_input]:bg-admin-bg [&_input]:px-3 [&_input]:py-2 [&_input]:text-admin-ink">{children}</div><ErrorText message={error} /></div> }
