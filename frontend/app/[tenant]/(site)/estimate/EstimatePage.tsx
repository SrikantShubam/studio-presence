    ...
    <article className="overflow-x-clip bg-surface text-ink">
      ...
      <HomeSection>
        <section className={`${pagePad} pb-[clamp(64px,9vw,120px)]`}>
          <EstimateCalc estimate={estimate} phone={site.business.phone} hours={site.business.hours} />
        </section>
      </HomeSection>
          ...
          <section className={`${pagePad} border-t border-accent bg-panel py-[clamp(64px,9vw,110px)]`}>
            <div className="mb-[clamp(36px,5vw,64px)] flex flex-wrap items-end justify-between gap-6">
              <h2 className="m-0 font-display text-[clamp(36px,6.5vw,84px)] font-light uppercase leading-[0.88] tracking-[-0.03em] text-ink">
                ...
                <ClipLine className="ml-[0.55em] block text-accent" delay={0.08}>
                  {copy.includedTitle.accent}
                </ClipLine>
              </h2>
              <Link href="/#services" className="text-[11.5px] font-medium uppercase tracking-[0.2em] text-ink hover:text-accent">
                {copy.seeServices} ↗
              </Link>
            </div>
              ...
              {estimate.included.map((item, i) => (
                <StaggerItem key={item.title}>
                  <div className="grid gap-3.5 border-t border-accent pt-[22px]">
                    ...
                    <h3 className="m-0 text-[clamp(15px,1.6vw,18px)] font-normal uppercase tracking-[0.06em]">{item.title}</h3>
