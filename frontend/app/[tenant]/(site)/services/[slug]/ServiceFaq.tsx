          ...
          {faq.map((item, index) => {
            ...
            return (
              <FadeUpItem key={item.q} className="border-b border-accent">
                ...
                >
                  <span className="min-w-0 break-words text-[clamp(15px,1.8vw,21px)] font-normal uppercase tracking-[0.01em]">
                    {item.q}
                  </span>
                ...
                <AnimatePresence initial={false}>
                  {isOpen ? (
                    ...
                    >
                      <p className="m-0 pb-[clamp(20px,2.4vw,30px)] pr-[clamp(0px,8vw,90px)] text-pretty text-justify text-[15px] leading-[1.75] text-body">
                        {item.a}
                      </p>
                    </motion.div>
                  ) : null}
