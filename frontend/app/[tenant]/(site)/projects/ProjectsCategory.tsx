...
const COPY = {
  ...
  title: { lead: 'Our', accent: 'Portfolio' },
  cta: {
    eyebrow: ['Seen something', 'close to yours?'],
    title: { lead: 'Start', accent: 'Your own' },
    action: 'Calculate the estimate',
  },
}

function ListingCta({ estimateEnabled }: { estimateEnabled: boolean }) {
  const href = estimateEnabled ? '/estimate' : '#footer'
...
export function ProjectsCategory({
  ...
  return (
    <article className="overflow-x-clip bg-surface text-ink">
        ...
        <div className="flex flex-wrap items-end justify-between gap-[clamp(24px,4vw,56px)]">
          <div className="min-w-0">
            ...
            <h1 className="m-0 font-display text-[clamp(44px,8.5vw,108px)] font-light uppercase leading-[0.88] tracking-[-0.03em] text-ink">
              ...
              <ClipLine className="ml-[0.55em] text-accent" delay={0.08}>{COPY.title.accent}</ClipLine>
            </h1>
          </div>
          {portfolio.introText ? (
            <FadeUp>
      ...
      <HomeSection>
        <ListingCta estimateEnabled={Boolean(site.sections.estimate?.enabled)} />
      </HomeSection>
