...
const mono = JetBrains_Mono({ subsets: ['latin'], display: 'swap', variable: '--font-dashboard-mono' })
...
export default async function DashboardLayout({
  ...
  return (
    <div className={`${inter.className} ${mono.variable} text-[13px]`}>
      <DashboardShell tenant={tenantSlug} studioName={branding.business.name} ownerName={branding.business.ownerName ?? ''} authenticated={isAuthenticated} signOutAction={signOut}>
        {children}
      </DashboardShell>
    </div>
  )
